...
int outputHash[] = { 0, 24464, 35681, 27159, 41456, 48282 };
vector<string> OutputH;
vector<set<pair<int, int>>> InputE;
vector<int> InputN, InputM, InputS, InputD, InputK;
vector<vector<int>> InputU, InputV, OutputP;
...
inline void prepareInput() {
    ...
    if (batch == 1) {
        ...
        InputD = { 3, 5 };
        InputK = { 5, 3 };
        InputU = { {2, 5, 4, 3, 1, 2, 3, 1, 1, 3}, {5, 3, 2, 2} };
        InputV = { {5, 1, 5, 5, 2, 4, 2, 4, 3, 4}, {1, 1, 3, 4} };
    }
    else if (batch == 2) {
        InputN = { 8, 6 };
        InputM = { 7, 6 };
        ...
        InputD = { 2, 2 };
        InputK = { 4, 2 };
        InputU = { {7, 6, 7, 2, 1, 8, 5}, {5, 1, 1, 5, 4, 3} };
        InputV = { {2, 7, 3, 4, 2, 4, 1}, {1, 2, 4, 2, 5, 2} };
    }
    else {
        InputN.resize(nTest[batch]);
        InputM.resize(nTest[batch]);
        InputS.resize(nTest[batch]);
        InputD.resize(nTest[batch]);
    ...
    for (fout << nTest[batch] << "\n", test = 0; test < nTest[batch]; ++test) {
        fout << InputN[test] << " " << InputM[test] << " " << InputS[test]
            << " " << InputD[test] << " " << InputK[test] << "\n";
...
inline void validateOutput() {
    try {
        ...
        int l = 0, n;
        OutputH.clear();
        OutputP.resize(nTest[batch]);
        for (ifstream fin("out.txt"); fin >> word; ) {
            assertThrow(sscanf(word.c_str(), "%d%c", &n, &c) == 1);
            if (l) --l, OutputP[test].push_back(n);
            else assertThrow(nTest[batch] > ++test),
        ...
        for (test = 0; test < nTest[batch]; ++test)
            if (!OutputP[test].empty()) {
                assertThrow(OutputP[test][0] == InputS[test]),
                    assertThrow(OutputP[test].back() == InputD[test]),
                    assertThrow(find(OutputP[test].begin(), OutputP[test].end(),
                        InputK[test]) != OutputP[test].end());
                for (int i = OutputP[test].size(); --i;) assertThrow(
                    InputE[test].find(make_pair(OutputP[test][i - 1],
                        OutputP[test][i])) != InputE[test].end());
            }
    }
    catch (...) {
