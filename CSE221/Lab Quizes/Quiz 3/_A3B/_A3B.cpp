...
int nTest[] = { 0, 2, 2, 10000, 20, 2 };
int maxN[] = { 0, 10, 10, 20, 10000, 100000 };
int maxAi[] = { 0, 10, 100, 1000, 100000, 1000000 };
int outputHash[] = { 0, 12388, 13926, 28202, 61779, 56198 };
...
inline void prepareInput() {
    if (batch == 1) {
        InputN = { 2, 5 };
        InputA = { {1, 2}, { 5, 4, 3, -2, -1 } };
    }
    else if (batch == 2) {
        InputN = { 6, 1 };
        InputA = { {10, 2, 5, 1, -2, 25}, {-10} };
    }
    else {
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            ...
            for (int i = 0; i < N; ++i)
                InputA[test][i] = getRandInt(-maxAi[batch], maxAi[batch]);
        }
    }
