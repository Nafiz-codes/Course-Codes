...
int outputHash[] = { 0, 49295, 26776, 27703, 60393, 43408 };
...
inline void prepareInput() {
    ...
    if (batch == 1) {
        ...
        InputS = { 2, 3 };
        InputD = { 3, 4 };
        InputU = { {2, 2, 3}, {1, 1, 1, 2, 4} };
        InputV = { {1, 3, 4}, {2, 4, 6, 6, 6} };
        InputW = { {3, 5, 3}, {3, 5, 2, 3, 1} };
    }
    else if (batch == 2) {
        InputN = { 5, 2 };
        InputM = { 7, 1 };
        InputS = { 2, 2 };
        InputD = { 5, 2 };
        InputU = { {1, 1, 5, 5, 5, 3, 3}, {2} };
        InputV = { {5, 4, 3, 4, 2, 4, 2}, {1} };
        InputW = { {3, 2, 1, 6, 5, 4, 8}, {5} };
    }
    else {
        ...
        for (test = 0; test < nTest[batch]; ++test) {
            ...
            if (1 & getRandInt(0, 1)) --M,
                ...
                InputE[test].insert(make_pair(D, S));
            while (M--) {
    ...
    for (fout << nTest[batch] << "\n", test = 0; test < nTest[batch]; ++test) {
        fout << InputN[test] << " " << InputM[test] << " "
            << InputS[test] << " " << InputD[test] << "\n";
        for (int i = 0; i < InputM[test]; ++i) fout << InputU[test][i]
